...
/**
 * Результаты участников команды за выбранный сезон, результаты
 * которых учитываются в рейтинге среди команд.
 */
export const fetchTeamParticipantRatingResults = createAsyncThunk(
  'teamParticipantRatingResults/fetch',
  async function ({ seasonLabel, teamUrlSlug }, thunkAPI) {
    try {
      const response = await myAxios({
        url: `${serverExpress}/api/teams/rating-results/${teamUrlSlug}/${seasonLabel}`,
        method: 'get',
      });
