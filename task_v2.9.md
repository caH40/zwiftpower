...
----
-сделать отдельный маршрут, контролер и сервис для страницы Редактирование Эвента;
...
-в схеме ZwiftEventSchema есть organizer: string; , проверить где он используется. Добавлена ссылка на документ Организатора, поэтому может нет надобности в этом свойстве?;  
