...
export const useSeriesTableListActions = ({ organizerId }) => {
  ...
  const deleteSeries = async (seriesId, name) => {
    const confirmed = window.confirm(
      `Вы действительно хотите удалить Серию заездов с названием "${name}". Будут удалены все данные связанные с этой серии: результаты этапы, генеральная классификация.`
    );
