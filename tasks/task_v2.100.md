...
2.125.52 +исправлен баг дублирования активности в фит файле райдера. Изменен срок хранения данных мощности с 90 до 60 дней;  
2.125.53 +исправлен баг пересчета результатов этапов у которых выключено правило общего времени на финише;  
2.125.54 +исправлен баг при изменении параметров этапа удалялись, но заново не создавались результаты этапа;  
2.125.55 +при запросе на пересчет ГК пересчитываются и все этапы;  
2.125.56 +исправлен баг при изменении номера этапа у эвента;  
2.125.57 +не учитываются результаты этапа в генеральной классификации если стоит соответствующий флаг в параметрах этапа серии;  
2.125.58 +добавлено новое поле в сущность этапа в серии заездов - requiredForGeneral;  
2.125.59 +добавлено поле requiredForGeneral в форме редактирования этапа в серии;  
2.125.60 +доработан сервис расчета ГК с учетом нового поля requiredForGeneral в этапах в серий;  
2.125.61 +форма ручного добавления результата в финишный протокол этапа серии;  
2.125.62 +редуктор отправки данных результата в финишный протокол этапа серии;  
2.125.63 +в форму нового результата на этапе добавлены поля описания райдера;  
2.125.64 +исправлен баг при сохранении фитфайла в БД после обновления результатов Эвентов;  
2.125.65 +метод формирования объекта результата из данных с клиента;  
2.125.66 +метод добавления нового результата в протокол этапа серии заездов;  
2.125.67 +сервис, контролер, маршрут удаления результата этапа серии, который был добавлен модератором;  
2.125.68 +функционал удаления результата этапа серии, который был добавлен модератором на клиенте;  
2.125.69 +добавление команды в результат этапа, добавляемого модератором;  
2.125.70 +добавлено отображение иконки добавлено модератором в соответствующем результате;   
2.125.71 +исправлен баг при ручном обновлении протокола эвента;   
2.125.72 +сервис, контроллер и маршрут для установки/удаления дисквалификации результата этапа серии;   
2.125.73 +изменен тип дисквалификации, убрано поле status;   
2.125.74 +фиксация багов в сервисах тура после изменения типа дисквалификации, убрано поле status;   
2.125.75 +изменен лейбл дисквалификации CUT на LMT - превышен лимит времени;  
2.125.76 +при дисквалификации на на этапе происходит дисквалификация в ГК;  
2.125.77 +отображение временного штрафа в таблицы результатов этапа серии;  
2.125.78 +исправлены мелки баги;  
2.125.79 +открыт доступ к финишным протоколам с zwiftAPI для пользователей;  
2.125.80 +исправлен баг при открытии линка регистрации в заезд в safari браузере;  
2.125.81 +исправлены мелкие баги;  
2.125.82 +обновлены данные по маршрутам и джерси;  
2.126.1 +общий лейаут для всех страниц с боковой панелью для виджетов;  
2.126.2 +файл конфигурации отображения боковой панели и виджетов, реклам в ней;  
2.126.3 +обновлен Browserslist;  
2.126.4 +мелкие правки;  
2.126.5 +добавлен банер рекламы goprotect;  
2.126.6 +небольшие правки;  
2.126.7 +при несуществующей странице осуществляется редирект на страницу 404 с заменой path на 404;  
2.126.8 +исправлен баг при создании велосипедного маршрута размер запроса которого превышал 10мегабайт;  
2.126.9 +добавление информации об ошибки при загрузке gxp трека из облака;  
2.126.10 +увеличено время ожидания ответа от сервера погоды до 1500мс;  
2.126.11 +компонент слайдера внизу страницы для отображения рекламы;  
2.126.12 +мелкие правки в банере рекламы;  
2.126.13 +добавлено новое голосование на заезды эндуренс;  
2.126.14 +исправлен баг при определении обязательных и завершенных этапов в туре;  
2.126.15 +исправлен баг при отображении лейбла дисквалификации в результатах заезда и результатах райдера;  
2.126.16 +исправлен баг при обновлении результатов заезда в котором проехал райдер с несуществующим zwiftId в БД;  
2.126.17 +изменено изображение рекламного баннера goprotect;  
2.126.18 +изменено изображение рекламного баннера goprotect;  
2.127.1 +обновлены библиотеки routes, jerseys;  
2.127.2 +удален блок донатов;  
2.128.1 +удалены скрипты загрузки рекламы яндекса;  
2.128.2 +исправлен баг активации подписки когда дата старта подписки еще не настала;  
2.128.3 +исправлен баг активации подписки когда дата старта подписки еще не настала часть 2;  
2.128.4 +разделение активных слотов организаторов дополнительно на не стартовавшие;  




----
-сделать документацию по настройкам tags;  
-добавить в title, descritpion слово звифт на страницах команды для лучшей индексации;  
-FIXME: отображать в финишном протоколе CP 15c, 1,5,20 из протокола звифта, а не расчетные из фитфайла;  
-оптимизировать компоненты TdRank и Rank, одинаковые.
-при дисквалификации на обязательно этапе серии дисквалифицировать в ГК.

-исключить запросы в zwiftAPI для обновления результатов при изменении правил там где это можно;

-в таблице главного зачета для результатов которые не попали в лимит отображать соответствующую иконку, вместо кубка на первую тройку; Сделать чкбокс включения дополнительной информации такой как, кубок, не попадание в лимит, пенальти и .т.д 
